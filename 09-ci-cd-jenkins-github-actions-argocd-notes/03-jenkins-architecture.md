...
Coordinates jobs and pipeline execution.
...
A directory where a job checks out and works with source.

## Credentials

Jenkins provides a credential-management mechanism so secrets do not need to be hard-coded in pipelines.
...
A Jenkins installation can become difficult to maintain if it depends on too many unmanaged plugins.

Keep plugins reviewed and pipelines in source control.
...
### Practical lab: understand Jenkins controller/agent execution
...
A pipeline that builds Docker images or runs infrastructure commands should execute on an appropriately configured agent.

When a build says "command not found", I first ask:

> Which agent executed this stage?
...
The pipeline environment is not automatically the same as my laptop.
...
For **Jenkins Architecture**, the workflow I want to remember is:
...
```text
jenkins, agent, docker, pipeline, change, kubernetes, result, stage, text, verify, controller, evidence
```
...
> "The reason we use Jenkins Architecture here is because it gives us a repeatable way to handle jenkins architecture. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
