...
A secret can leak even if the pipeline itself is "private."

## Principle

A pipeline has powerful permissions.

If CI can deploy production, compromise of CI can become compromise of production.
...
### Practical lab: audit pipeline secrets

I search my repository for accidental secret patterns without printing real values.
...
In GitHub Actions I use repository/environment secrets or workload identity mechanisms where appropriate.

In Jenkins I use its credential store.

I follow the rule:

```text
secret → secret manager / CI credential mechanism
never → source code
```

If a secret is accidentally committed, deleting the line in a later commit does not automatically make the secret safe. I treat it as exposed and rotate it.
...
For **Ci Cd Secrets Security**, the workflow I want to remember is:
...
```text
secret, pipeline, credentials, secrets, change, keys, result, text, verify, changes, check, cloud
```
...
> "The reason we use Ci Cd Secrets Security here is because it gives us a repeatable way to handle ci cd secrets security. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
