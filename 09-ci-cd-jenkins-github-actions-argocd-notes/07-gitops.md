...
GitOps uses Git as a source of truth for declarative desired state.
...
Argo CD observes Git and reconciles the cluster.
...
> "Everything must happen in GitHub Actions."

CI and GitOps can have separate responsibilities.
...
### Practical lab: separate CI from deployment

I use two conceptual repositories:
...
CI changes the desired image version in the GitOps repository.

Argo CD observes Git and reconciles Kubernetes.

This gives me a clean audit trail:

```text
commit → image → GitOps commit → cluster
```

I do not make the CI server directly run `kubectl apply` when I intentionally want a GitOps model.
...
For **Gitops**, the workflow I want to remember is:
...
```text
gitops, argo, text, image, kubernetes, change, changes, deployment, result, verify, apply, cluster
```
...
> "The reason we use Gitops here is because it gives us a repeatable way to handle gitops. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
