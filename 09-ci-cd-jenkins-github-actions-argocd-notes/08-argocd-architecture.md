...
Argo CD is a Kubernetes continuous delivery tool built around GitOps.
...
If live state differs, Argo CD can identify the difference and, according to sync configuration, reconcile it.
...
Argo CD is not the container registry.

It does not replace Docker/ECR.

It is responsible for delivery/reconciliation of Kubernetes application state.
...
### Practical lab: inspect Argo CD's reconciliation model

After Argo CD is installed, I identify:
...
- What Git revision is Argo CD watching?
- What path is it rendering?
- Which cluster/namespace is the Application targeting?
- Is the live state Synced?
- Is it Healthy?
- If not, which resource differs?

The practical skill is learning to explain the difference between **Git desired state** and **live cluster state**.
...
For **Argocd Architecture**, the workflow I want to remember is:
...
```text
argo, state, kubernetes, application, live, text, change, cluster, desired, identify, result, verify
```
...
> "The reason we use Argocd Architecture here is because it gives us a repeatable way to handle argocd architecture. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
