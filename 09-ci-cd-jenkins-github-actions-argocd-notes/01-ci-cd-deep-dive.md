...
The output remains deployable.

A human approval may still exist before production.

## Continuous Deployment

Successful changes are deployed automatically.
...
```text
Continuous Delivery:
Ready to deploy automatically or with approval.

Continuous Deployment:
Actually deploys automatically.
```

## Pipeline principle

The pipeline should fail fast on important quality/security problems while keeping logs useful enough to troubleshoot.
...
### Practical lab: build the delivery pipeline on paper first

Before writing Jenkins/GitHub Actions syntax, I define the stages:
...
This prevents the pipeline from becoming a long script where nobody knows which stage is responsible for a failure.
...
For **Ci Cd Deep Dive**, the workflow I want to remember is:
...
```text
pipeline, continuous, stage, text, verify, before, change, changes, delivery, deployment, result, test
```
...
> "The reason we use Ci Cd Deep Dive here is because it gives us a repeatable way to handle ci cd deep dive. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
