...
An Application describes where desired Kubernetes state comes from and where it should be deployed.
...
The Application can be synchronized manually or automatically depending on policy.
...
### Practical lab: deploy one application through an Argo CD Application

I define an Application that points to:
...
After creating it, I inspect the resulting resources:
...
Then I make a harmless change in Git and observe:
...
This is the practical behavior I want to understand before using automated sync.
...
For **Argocd Application**, the workflow I want to remember is:
...
```text
application, change, argo, destination, kubernetes, namespace, path, cluster, result, text, verify, before
```
...
> "The reason we use Argocd Application here is because it gives us a repeatable way to handle argocd application. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
