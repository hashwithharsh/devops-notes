...
Same source should produce the same expected result.

### Observable

Logs and status clearly show what happened.
...
Feedback should arrive quickly enough to encourage frequent changes.

### Reversible

Production deployments should have a rollback/recovery strategy.

## Artifact principle

Build an artifact once and promote that artifact.
...
Avoid rebuilding a different image for every environment without a reason.
...
### Practical: design a pipeline I could actually troubleshoot

I prefer small stages:
...
If the build fails, later deployment stages should not run.

If deployment succeeds but smoke tests fail, the pipeline should clearly show that the application was deployed but verification failed.

I also make the image tag traceable to the Git commit:

```text
Git SHA → image tag → deployment manifest
```

That makes rollback and investigation much easier.
...
For **Pipeline Design**, the workflow I want to remember is:
...
```text
pipeline, image, text, build, deployment, test, verify, change, result, actually, artifact, commit
```
...
> "The reason we use Pipeline Design here is because it gives us a repeatable way to handle pipeline design. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
