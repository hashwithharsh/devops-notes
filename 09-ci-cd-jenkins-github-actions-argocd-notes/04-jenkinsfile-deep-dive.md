...
Pipeline configuration becomes:
...
```groovy
sh "curl -u user:password ..."
```

Use Jenkins credentials mechanisms.
...
### Practical lab: write a small Jenkins pipeline
...
I keep credentials out of the Jenkinsfile and use Jenkins credentials mechanisms.

When a stage fails, I inspect the console output and identify the first command that returned non-zero status rather than focusing only on the final "pipeline failed" message.
...
For **Jenkinsfile Deep Dive**, the workflow I want to remember is:
...
```text
pipeline, stage, steps, credentials, test, build, change, docker, image, jenkins, jenkinsfile, result
```
...
> "The reason we use Jenkinsfile Deep Dive here is because it gives us a repeatable way to handle jenkinsfile deep dive. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
