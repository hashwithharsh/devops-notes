...
A repository is a source from which Linux obtains packages.
...
Can be backed by ISO content or an internal repository server.
...
## Temporary repository selection

```bash
sudo dnf install tree --disablerepo="*" --enablerepo="Offline-*"
```

This affects that command only.
...
The advantage is control and predictability.

## Priority

Repository priority can prevent an unintended third-party repository from overriding an approved source.

Lower priority number means higher priority when the configured plugin behavior applies.

## Version locking

Version locking can prevent accidental upgrades of critical packages.

## Production principle

**Production package management should be controlled, auditable and predictable.**
...
### Practical lab: inspect repository configuration safely

I inspect enabled repositories:

```bash
dnf repolist
```

Then I inspect repository files:
...
I do not immediately overwrite repository files. First I understand:

- repository ID
- base URL/mirror
- enabled state
- GPG checking
- whether the server can reach the repository

I can test metadata refresh:
...
If a package is missing, I troubleshoot:
...
Can the host reach it?
        ↓
Is the package available for this OS/version?
        ↓
Is GPG validation working?
        ↓
Are dependencies available?
```

This same sequence works for internal enterprise repositories.
...
For **Repository Management Enterprise**, the workflow I want to remember is:
...
```text
repository, bash, server, sudo, text, verify, change, enabled, enterprise, inspect, priority, result
```
...
> "The reason we use Repository Management Enterprise here is because it gives us a repeatable way to handle repository management enterprise. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
