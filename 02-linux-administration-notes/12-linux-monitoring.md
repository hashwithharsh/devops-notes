# Linux Monitoring and Resource Troubleshooting
...
Do not look only at "free memory." Linux uses memory for caching too.
...
`df` answers filesystem capacity questions.

`du` helps identify which files/directories consume space.
...
Always correlate metrics with logs and recent changes.
...
### Practical: investigate a slow Linux server

I begin with a snapshot:
...
Then I identify CPU-heavy processes:
...
Disk and I/O questions need different evidence, so I use tools available on the system, such as:
...
**My troubleshooting model:**

```text
CPU → memory → disk space/inodes → I/O → network → application
```

I do not assume "server slow" means CPU is high.
...
For **Linux Monitoring**, the workflow I want to remember is:
...
```text
bash, disk, sort, verify, change, head, memory, network, result, symptom, text, troubleshooting
```
...
> "The reason we use Linux Monitoring here is because it gives us a repeatable way to handle linux monitoring. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
