...
check logs/application behavior
 ↓
determine whether expected or abnormal
 ↓
...
Then inspect logs, deleted-but-open files and application data.
...
Check configuration, dependencies, ports, permissions and recent changes.
...
Then validate the fstab entry with `mount -a`.
...
Then investigate repository, dependency, network or package conflicts.
...
For **Linux Interview Scenarios**, the workflow I want to remember is:
...
```text
check, verify, text, bash, change, explain, hypothesis, mount, network, result, service, symptom
```
...
> "The reason we use Linux Interview Scenarios here is because it gives us a repeatable way to handle linux interview scenarios. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
