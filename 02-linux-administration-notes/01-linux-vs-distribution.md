...
A distribution packages the kernel with other software needed to create a usable operating system.
...
`/etc/os-release` provides distribution identification information.
...
### Practical: identify what Linux I am actually running

When I log into an unfamiliar server, I first collect basic facts instead of assuming the distribution.
...
Then I check the package manager:
...
I also check the filesystem and disks:
...
**What I am looking for:** kernel version, distribution/version, hostname, package-management family and filesystem layout.

This is useful before giving commands to another server because a command that is correct on RHEL/Rocky/Alma may not be the right command on Debian/Ubuntu.
...
For **Linux Vs Distribution**, the workflow I want to remember is:
...
```text
command, distribution, verify, bash, change, check, kernel, result, would, actually, before, create
```
...
> "The reason we use Linux Vs Distribution here is because it gives us a repeatable way to handle linux vs distribution. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
