# DNF and RPM Practical Notes

This note summarizes the operational relationship between DNF and RPM.

## DNF

Use DNF for normal package operations:
...
Use RPM for package-level inspection and deeper debugging:
...
**Use DNF for package management. Use RPM when you need package-level inspection/debugging.**
...
Avoid forcing an installation with dependency bypass options on production systems unless you fully understand the consequences.

## Security patching

On supported RHEL-family systems, security update information can be inspected with DNF updateinfo tooling.

A production patch process should be:
...
Rollback availability depends on package versions and repository availability.
...
### Practical lab: investigate a package from start to finish
...
Find files installed by it:

```bash
rpm -ql nginx
```

If I want to know which package owns a file:

```bash
rpm -qf /usr/sbin/nginx
```

Then I manage the service separately:
...
The practical distinction I remember is:

```text
DNF → dependency-aware package management
RPM → package inspection/low-level package operations
...
For **Dnf Rpm Practical**, the workflow I want to remember is:
...
```text
nginx, bash, package, verify, change, sudo, install, result, test, text, would, check
```
...
> "The reason we use Dnf Rpm Practical here is because it gives us a repeatable way to handle dnf rpm practical. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
