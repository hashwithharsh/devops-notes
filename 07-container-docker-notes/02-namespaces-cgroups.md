...
Namespaces provide isolation of things such as:
...
A container can therefore have its own process/network view.

## cgroups

Control groups manage and account for resources such as:
...
When a container says:

> "It cannot see process X."

or:

> "The container is being killed for memory."

understanding namespaces/cgroups gives you a deeper explanation than simply memorizing Docker commands.
...
### Practical lab: observe container isolation
...
For resource controls I can apply a memory limit:

```bash
docker run --rm --memory=128m nginx
```

The practical idea is that namespaces provide isolation views while cgroups control resource usage. Docker exposes a higher-level interface over these Linux primitives.
...
For **Namespaces Cgroups**, the workflow I want to remember is:
...
```text
docker, inspect, container, namespaces, network, process, cgroups, bash, change, result, verify, explain
```
...
> "The reason we use Namespaces Cgroups here is because it gives us a repeatable way to handle namespaces cgroups. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
