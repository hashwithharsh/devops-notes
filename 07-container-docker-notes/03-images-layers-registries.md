...
An image is a template from which containers are created.

## Layers

Docker images are built from layers.

Each Dockerfile instruction can contribute to the image structure.
...
docker tag myapp:1.0 registry.example/myapp:1.0
docker push registry.example/myapp:1.0
```

## Production principle

Use meaningful immutable version tags for releases.
...
because it does not communicate which exact build is being deployed.
...
### Practical lab: inspect an image
...
```bash
docker tag nginx:stable my-nginx:lab
```

The important lesson is that a tag is a human-friendly reference. For reproducibility I care about the exact image version/digest used by the deployment.

For a registry workflow the normal path is:
...
For **Images Layers Registries**, the workflow I want to remember is:
...
```text
docker, image, inspect, nginx, network, bash, change, images, only, pull, result, stable
```
...
> "The reason we use Images Layers Registries here is because it gives us a repeatable way to handle images layers registries. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
