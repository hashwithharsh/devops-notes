...
`-p 8080:80` publishes it through Docker's networking configuration.
...
### Practical lab: connect two containers
...
```bash
docker run -d --name web --network app-net nginx
docker run --rm -it --network app-net busybox sh
```

From the temporary container, test the service by container name where supported:

```sh
wget -qO- http://web
```

The practical point is that user-defined networks provide service-to-service connectivity and name resolution without requiring me to hard-code container IP addresses.
...
For **Networking**, the workflow I want to remember is:
...
```text
docker, network, app-net, bash, container, inspect, port, change, containers, create, name, result
```
...
> "The reason we use Networking here is because it gives us a repeatable way to handle networking. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
