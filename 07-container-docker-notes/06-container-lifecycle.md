...
A container is usually treated as disposable.

Persistent application data should normally live outside the container writable layer.
...
### Practical lab: follow the container lifecycle
...
I remember the states as:

```text
created → running → stopped → removed
```

`docker exec` enters a running container; it does not create a second container.

When debugging I normally start with:
...
For **Container Lifecycle**, the workflow I want to remember is:
...
```text
docker, container, inspect, logs, bash, network, change, result, start, text, verify, does
```
...
> "The reason we use Container Lifecycle here is because it gives us a repeatable way to handle container lifecycle. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
