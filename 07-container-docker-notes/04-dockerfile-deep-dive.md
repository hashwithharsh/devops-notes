...
Defines the executable behavior more strongly than CMD and is often combined with CMD for default arguments.

## EXPOSE

Documents the intended container port. It does not itself publish the port to the host.
...
### Practical lab: build my own image

For a small Python application, a basic Dockerfile might be:
...
```bash
docker build -t devops-notes-app:1.0 .
```

Run:

```bash
docker run --rm -p 5000:5000 devops-notes-app:1.0
```
...
I then improve the Dockerfile instead of stopping at "it runs": non-root user, smaller runtime image, `.dockerignore`, pinned dependencies and a health-check strategy.
...
For **Dockerfile Deep Dive**, the workflow I want to remember is:
...
```text
docker, build, dockerfile, copy, image, inspect, network, python, bash, change, logs, requirements.txt
```
...
> "The reason we use Dockerfile Deep Dive here is because it gives us a repeatable way to handle dockerfile deep dive. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
