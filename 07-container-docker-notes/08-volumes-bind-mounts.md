...
Data written only inside the container can disappear when the container is removed.
...
Volumes are useful for persistent application data.

Bind mounts are useful when the host path itself needs to be controlled, often during development or specific operational use cases.

## Security

Be careful about mounting sensitive host directories into containers.
...
### Practical lab: persist data

Create a named volume:

```bash
docker volume create app-data
docker run --rm -v app-data:/data busybox sh -c 'echo hello > /data/test.txt'
docker run --rm -v app-data:/data busybox cat /data/test.txt
```

The second container can read the data because the data lives in the volume, not only in the first container's writable layer.

For development, a bind mount can map a host directory:

```bash
docker run --rm -v "$PWD":/app busybox ls -la /app
```

I choose volumes for managed persistent container data and bind mounts when I intentionally need a host path.
...
For **Volumes Bind Mounts**, the workflow I want to remember is:
...
```text
docker, data, container, bind, host, app-data, network, only, volume, bash, change, inspect
```
...
> "The reason we use Volumes Bind Mounts here is because it gives us a repeatable way to handle volumes bind mounts. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
