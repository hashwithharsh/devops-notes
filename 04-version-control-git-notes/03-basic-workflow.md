...
Do not think of `git add` as "saving the file." It prepares content for the next commit.
...
### Practical lab: complete the normal Git cycle
...
I deliberately edit the file without staging it:
...
Stage it and inspect the staged diff:
...
This is the workflow I want to be able to visualize:
...
For **Basic Workflow**, the workflow I want to remember is:
...
```text
bash, commit, repository, status, change, create, push, readme.md, result, staging, state, text
```
...
> "The reason we use Basic Workflow here is because it gives us a repeatable way to handle basic workflow. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
