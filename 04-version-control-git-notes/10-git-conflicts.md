# Git Merge and Rebase Conflicts

A conflict occurs when Git cannot automatically determine which changes should remain.
...
Understand what each side of the change was trying to accomplish.
...
### Practical lab: resolve a conflict methodically

When Git reports a conflict, I do not immediately delete markers.
...
I decide what the final file should contain. Then I remove the markers and test the file.
...
For a merge conflict, the operation normally finishes with a merge commit unless fast-forwarding is possible.

The important practical skill is being able to explain *why* the conflict occurred: two histories changed overlapping content and Git could not safely choose one automatically.
...
For **Git Conflicts**, the workflow I want to remember is:
...
```text
bash, conflict, change, merge, rebase, commit, file, branch, changes, current, resolve, result
```
...
> "The reason we use Git Conflicts here is because it gives us a repeatable way to handle git conflicts. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
