...
Use `-D` carefully because it can delete an unmerged branch.
...
### Practical lab: feature branch workflow
...
```bash
git add .
git commit -m "feat: add login notes"
```
...
The important part is not the command itself. I want to understand that a branch is essentially a movable reference to commits.

I use descriptive branch names and keep branches short-lived so integration does not become unnecessarily difficult.
...
For **Branches**, the workflow I want to remember is:
...
```text
branch, feature, login, bash, branches, change, main, switch, command, commit, reference, result
```
...
> "The reason we use Branches here is because it gives us a repeatable way to handle branches. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
