...
## Pull Request should communicate
...
GitHub is a collaboration platform around Git; Git itself is the version control system.
...
### Practical lab: simulate a Pull Request workflow
...
Then I open a Pull Request on GitHub.

Before merging I expect:
...
After merge, I update my local main branch:
...
The `--ff-only` habit helps me notice unexpected local divergence instead of silently creating a merge commit.
...
For **Github Pr Workflow**, the workflow I want to remember is:
...
```text
pull, branch, merge, before, change, commit, feature, request, workflow, main, result, text
```
...
> "The reason we use Github Pr Workflow here is because it gives us a repeatable way to handle github pr workflow. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
