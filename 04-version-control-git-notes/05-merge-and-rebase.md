...
Merging combines the histories and can create a merge commit.
...
Rebase replays feature commits onto another base.
...
The rebased commits are new commits.
...
Because it rewrites commit history. Existing commit IDs can change, which can disrupt collaborators who already based work on the old history.
...
### Practical lab: create a real merge conflict

I create two branches from the same commit and change the same line differently.
...
I open the conflicted file, choose the correct content, then:
...
If I realize the rebase is wrong:

```bash
git rebase --abort
```

My practical rule: rebase is excellent for cleaning up private/local history; I avoid rewriting commits that other people are already depending on unless the team explicitly agrees.
...
For **Merge And Rebase**, the workflow I want to remember is:
...
```text
rebase, bash, merge, feature, main, change, commit, create, before, commits, conflict, file
```
...
> "The reason we use Merge And Rebase here is because it gives us a repeatable way to handle merge and rebase. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
