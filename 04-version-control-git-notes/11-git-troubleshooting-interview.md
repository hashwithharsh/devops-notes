# Git Troubleshooting and Interview Notes
...
Removing a secret from the latest commit does not automatically remove it from all historical commits.
...
## Need to undo production commit
...
- What does `git pull` do conceptually?
...
For **Git Troubleshooting Interview**, the workflow I want to remember is:
...
```text
commit, verify, change, does, hypothesis, result, secret, state, troubleshooting, accidentally, before, changes
```
...
> "The reason we use Git Troubleshooting Interview here is because it gives us a repeatable way to handle git troubleshooting interview. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
