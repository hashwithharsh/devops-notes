...
Every commit represents a point in history.
...
Centralized systems rely heavily on a central server.

Git is distributed. A clone normally contains repository history locally.
...
For **Vcs Concepts**, the workflow I want to remember is:
...
```text
commit, history, before, change, changes, repository, result, text, verify, check, command, control
```
...
> "The reason we use Vcs Concepts here is because it gives us a repeatable way to handle vcs concepts. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
