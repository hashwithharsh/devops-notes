...
resource "aws_s3_bucket" "example" {
  bucket = "example-devops-notes"
}
```

The first name is the resource type.

The second is the Terraform local name.

## Provider

A provider translates Terraform configuration into API operations for a platform.

Examples include AWS and other infrastructure/service providers.
...
### Practical lab: understand a resource from configuration to API

A minimal Terraform flow is:
...
I read the plan and identify the resource address, such as:

```text
aws_instance.web
```

I want to understand that Terraform configuration is not a shell script. Terraform builds a dependency graph and converges infrastructure toward the declared state.
...
For **Hcl Providers Resources**, the workflow I want to remember is:
...
```text
terraform, resource, change, understand, configuration, provider, state, plan, result, verify, apply, dependency
```
...
> "The reason we use Hcl Providers Resources here is because it gives us a repeatable way to handle hcl providers resources. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
