...
Terraform needs to know how configuration corresponds to real infrastructure.
...
State may contain resource information and, depending on configuration/provider behavior, sensitive values.
...
Do not casually edit `terraform.tfstate` manually.
...
For **State**, the workflow I want to remember is:
...
```text
terraform, state, change, verify, configuration, disposable, plan, result, understand, apply, before, changes
```
...
> "The reason we use State here is because it gives us a repeatable way to handle state. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
