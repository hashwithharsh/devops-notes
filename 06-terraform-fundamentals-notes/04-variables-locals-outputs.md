...
Locals store calculated/reused expressions.
...
Use locals for internal calculations.

Use outputs for values consumers need.

Do not turn every constant into a variable just because Terraform supports variables.
...
### Practical lab: make a configuration reusable
...
I use variables for inputs, locals for calculated/reused expressions, and outputs for values that another person or module needs to see.
...
For **Variables Locals Outputs**, the workflow I want to remember is:
...
```text
terraform, environment, locals, change, outputs, state, useful, variables, apply, output, result, understand
```
...
> "The reason we use Variables Locals Outputs here is because it gives us a repeatable way to handle variables locals outputs. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
