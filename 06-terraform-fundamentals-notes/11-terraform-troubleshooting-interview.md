# Terraform Troubleshooting and Interview
...
First determine whether another Terraform process is actually running.

## Resource exists but Terraform wants to create it

Possible causes:

- state does not contain the resource
...
For **Terraform Troubleshooting Interview**, the workflow I want to remember is:
...
```text
state, terraform, resource, change, verify, apply, does, hypothesis, inspect, plan, result, troubleshooting
```
...
> "The reason we use Terraform Troubleshooting Interview here is because it gives us a repeatable way to handle terraform troubleshooting interview. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
