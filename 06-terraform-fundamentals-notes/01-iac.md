...
Infrastructure as Code means describing infrastructure using machine-readable configuration rather than creating everything manually.
...
The problem is that the process is difficult to reproduce exactly.
...
You describe the desired infrastructure, and Terraform determines the actions needed to reach it.
...
### Practical lab: compare manual and IaC workflows

I imagine creating an EC2 environment manually:
...
Then I describe the same desired state in Terraform.

The important practical difference is that the infrastructure definition becomes reviewable:
...
For a lab I start with a small resource and destroy it after verification:
...
I always keep cost and cleanup in mind when practicing against AWS.
...
For **Iac**, the workflow I want to remember is:
...
```text
terraform, infrastructure, apply, change, state, text, click, plan, destroy, result, understand, verify
```
...
> "The reason we use Iac here is because it gives us a repeatable way to handle iac. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
