...
Terraform can usually infer dependencies from references.
...
Use explicit dependencies only when the dependency cannot be expressed naturally through references.
...
Other lifecycle behavior can change how Terraform handles replacement and protection.

## Important

Lifecycle settings can prevent or change destruction.

Understand the operational effect before using them.
...
### Practical lab: inspect dependency behavior

I create two resources where one references an attribute of the other.

Terraform can infer a dependency from the reference.

I inspect the plan and graph:

```bash
terraform graph
```

For lifecycle behavior, I read the proposed action carefully before applying.

If a resource is being replaced, I ask:

```text
What changed?
Is the provider forcing replacement?
Is the attribute immutable?
Can lifecycle settings safely change the behavior?
```

I do not add `create_before_destroy` just to make a plan look nicer. Lifecycle settings can materially change production behavior.
...
For **Dependencies Lifecycle**, the workflow I want to remember is:
...
```text
terraform, change, lifecycle, dependency, inspect, state, understand, before, behavior, dependencies, plan, result
```
...
> "The reason we use Dependencies Lifecycle here is because it gives us a repeatable way to handle dependencies lifecycle. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
