...
A single giant playbook becomes difficult to maintain.
...
Ansible Galaxy provides a mechanism for sharing/discovering Ansible roles and collections.

Before using external content in production, review:
...
### Practical lab: turn a playbook into a role

I create the role skeleton:

```bash
ansible-galaxy init nginx_role
```

I learn the purpose of the directories:
...
Then I move installation/configuration tasks into the role and call it:
...
I keep defaults in `defaults/main.yml` for values that should be easy to override.

The practical benefit is repeatability: the same role can configure development, staging and production with different variables.
...
For **Roles And Galaxy**, the workflow I want to remember is:
...
```text
role, roles, ansible, text, verify, change, configuration, defaults, playbook, result, state, tasks
```
...
> "The reason we use Roles And Galaxy here is because it gives us a repeatable way to handle roles and galaxy. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
