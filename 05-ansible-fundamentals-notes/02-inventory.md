...
Groups let you target related machines.
...
Inventory can contain connection information.

For larger environments, separate inventory/group variables from the host list.
...
Do not maintain a huge unstructured inventory file forever.

As environments grow, organize hosts by environment, role, region or other meaningful dimensions.
...
### Practical lab: build a small inventory
...
Then inspect the parsed inventory:

```bash
ansible-inventory -i inventory.ini --graph
```

I use this command whenever I suspect an inventory/grouping problem.
...
For **Inventory**, the workflow I want to remember is:
...
```text
inventory, ansible, inventory.ini, ansible_host, verify, bash, change, ping, result, state, ansible-inventory, configuration
```
...
> "The reason we use Inventory here is because it gives us a repeatable way to handle inventory. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
