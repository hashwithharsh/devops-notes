# Ansible Troubleshooting and Interview
...
Check SSH manually when necessary.
...
Check variable names, scope and precedence.

### Task says changed every run

Investigate whether the module/task describes state or simply executes a command.
...
- What is a role?
- What is Vault?
- How does `become` work conceptually?
- How do you troubleshoot an unreachable host?
...
For **Ansible Troubleshooting Interview**, the workflow I want to remember is:
...
```text
ansible, check, state, verify, bash, change, evidence, hypothesis, result, troubleshooting, configuration, does
```
...
> "The reason we use Ansible Troubleshooting Interview here is because it gives us a repeatable way to handle ansible troubleshooting interview. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
