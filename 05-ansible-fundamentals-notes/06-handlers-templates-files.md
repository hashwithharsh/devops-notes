...
The handler runs when the task reports a change.
...
Choose the module based on whether the file is static or needs templating.
...
### Practical lab: deploy a configuration and restart only when needed
...
The important behavior is that the handler runs when the template task reports a change. I therefore avoid restarting the service on every playbook run.
...
For **Handlers Templates Files**, the workflow I want to remember is:
...
```text
nginx, name, state, handlers, restart, verify, change, change., configuration, deploy, handler, result
```
...
> "The reason we use Handlers Templates Files here is because it gives us a repeatable way to handle handlers templates files. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
