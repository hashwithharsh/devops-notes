...
Ansible gathers information about the host.
...
Variables can come from many sources. In larger projects, document precedence and keep configuration organized so operators can understand why a value was selected.
...
### Practical lab: configure multiple hosts from variables

I can keep host-specific values in inventory or variable files.

Then a playbook can use:
...
I test the logic with:

```bash
ansible web -i inventory.ini -m setup
```

Facts are data Ansible gathered about the target, not magic variables I should guess blindly.
...
For **Variables Facts Conditionals Loops**, the workflow I want to remember is:
...
```text
yaml, state, variables, ansible, name, packages, verify, change, configuration, facts, loop, result
```
...
> "The reason we use Variables Facts Conditionals Loops here is because it gives us a repeatable way to handle variables facts conditionals loops. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
