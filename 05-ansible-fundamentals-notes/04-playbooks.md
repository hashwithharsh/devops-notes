...
A playbook describes automation in YAML.
...
### Practical lab: first useful playbook
...
The second run should not restart nginx unnecessarily. That is where I start seeing why Ansible's desired-state model is useful.
...
For **Playbooks**, the workflow I want to remember is:
...
```text
name, nginx, state, verify, ansible, ansible-playbook, bash, change, check, playbook, result, second
```
...
> "The reason we use Playbooks here is because it gives us a repeatable way to handle playbooks. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
