# Ansible Architecture and SSH

Ansible normally uses a control node to automate managed nodes.
...
The machine where Ansible is executed.
...
The managed Linux host normally does not require a permanent Ansible agent.

Ansible connects, performs the requested operation, and returns the result.
...
The `ping` module does not mean ICMP ping. It tests Ansible connectivity/module execution.
...
### Practical lab: connect to two Linux hosts
...
Then I verify Ansible can connect:

```bash
ansible all -i inventory.ini -m ping
```

If it fails, I troubleshoot in this order:
...
→ SSH key/user
→ sudo permissions
→ Python/runtime on target
```

I do not start debugging the playbook when `ansible -m ping` itself cannot reach the host.
...
For **Architecture And Ssh**, the workflow I want to remember is:
...
```text
ansible, verify, change, configuration, does, managed, node, permissions, ping, result, state, test
```
...
> "The reason we use Architecture And Ssh here is because it gives us a repeatable way to handle architecture and ssh. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
