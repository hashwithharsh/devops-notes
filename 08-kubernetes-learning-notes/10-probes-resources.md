...
Helps applications that take a long time to initialize.
...
Limits constrain resource usage according to Kubernetes/runtime behavior.

## Troubleshooting

If a Pod is repeatedly restarted:
...
### Practical lab: see readiness and liveness behave differently
...
> Should the kubelet restart this container?

I test readiness by making an application endpoint intentionally unavailable and observing Service traffic.
...
For resource behavior I inspect requests/limits:

```bash
kubectl get pod <pod-name> -o yaml
```

I remember that a bad liveness probe can create a restart loop, so probe settings should reflect actual application startup and health behavior.
...
For **Probes Resources**, the workflow I want to remember is:
...
```text
probe, kubectl, application, liveness, readiness, answers, change, failure, inspect, kubernetes, result, state
```
...
> "The reason we use Probes Resources here is because it gives us a repeatable way to handle probes resources. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
