...
RBAC controls who can perform which Kubernetes API actions.
...
Cluster-scoped permissions or reusable permissions that can be bound to namespace identities.

## RoleBinding

Connects a Role/ClusterRole to subjects in a namespace.

## ClusterRoleBinding

Binds ClusterRole permissions at cluster scope.
...
### Practical lab: verify an RBAC permission

I create a ServiceAccount:

```bash
kubectl create serviceaccount reader
```

Then I can test authorization explicitly:

```bash
kubectl auth can-i get pods --as=system:serviceaccount:default:reader
```

After creating the appropriate Role/RoleBinding, I run the same command again.

This gives me a very useful troubleshooting technique: `kubectl auth can-i` tells me whether the identity is authorized without needing to infer the answer from YAML.
...
For **Rbac Serviceaccounts**, the workflow I want to remember is:
...
```text
permissions, serviceaccount, verify, change, kubectl, kubernetes, rbac, result, role, state, text, clusterrole
```
...
> "The reason we use Rbac Serviceaccounts here is because it gives us a repeatable way to handle rbac serviceaccounts. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
