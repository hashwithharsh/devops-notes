...
A Deployment manages a desired state for replicated application Pods.
...
The Deployment manages a ReplicaSet, which manages Pods.
...
### Practical lab: create a Deployment and scale it
...
Inspect the controller relationship:
...
I then delete one Pod and watch Kubernetes create a replacement:
...
This is the reconciliation model in action.
...
For **Deployments Replicasets**, the workflow I want to remember is:
...
```text
deployment, kubectl, state, bash, change, pods, result, scale, verify, application, configuration, create
```
...
> "The reason we use Deployments Replicasets here is because it gives us a repeatable way to handle deployments replicasets. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
