...
In normal production applications, you generally create Pods through controllers such as Deployments rather than managing individual Pods manually.
...
### Practical lab: create and inspect a Pod

I create a small Pod:
...
I do not treat a Pod as the normal deployment unit for production applications. Controllers such as Deployments are normally responsible for keeping Pods at the desired count.
...
For **Pods**, the workflow I want to remember is:
...
```text
kubectl, nginx, pods, bash, change, inspect, kubernetes, result, state, verify, apply, check
```
...
> "The reason we use Pods here is because it gives us a repeatable way to handle pods. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
