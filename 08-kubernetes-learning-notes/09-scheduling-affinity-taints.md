# Kubernetes Scheduling, Affinity and Taints
...
More expressive node placement rules.
...
Prefer or require Pods to be separated.

## Taint

A node can reject Pods unless they tolerate its taint.

## Toleration

Allows a Pod to be considered for a tainted node.

## Example reasoning

Suppose database workloads should be isolated.
...
Taints/tolerations and affinity solve different parts of placement logic.

## Production principle

Do not add complex scheduling rules without a real requirement. Complexity itself becomes an operational cost.
...
### Practical lab: understand why a Pod is Pending
...
The Events section often tells me why scheduling failed.
...
If a Pod requires a label that no node has, or a node is tainted without a matching toleration, the scheduler can leave the Pod Pending.

I use events as evidence rather than guessing which scheduling rule is responsible.
...
For **Scheduling Affinity Taints**, the workflow I want to remember is:
...
```text
node, scheduling, affinity, events, change, database, kubectl, result, state, taints, verify, bash
```
...
> "The reason we use Scheduling Affinity Taints here is because it gives us a repeatable way to handle scheduling affinity taints. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
