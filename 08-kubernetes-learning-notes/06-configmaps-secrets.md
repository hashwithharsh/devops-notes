...
However, Kubernetes Secrets should not be treated as automatically safe simply because they are called secrets. Access control and encryption-at-rest practices matter.
...
### Practical lab: inject configuration
...
```bash
kubectl create secret generic app-secret --from-literal=DB_PASSWORD='<use-a-lab-only-value>'
```

I treat Kubernetes Secrets as sensitive configuration, not as automatically secure encrypted vaults. I also avoid putting real secrets into shell history or Git repositories.

When a Pod cannot see a configuration value, I verify the object exists in the same namespace and that the Pod references the correct key.
...
For **Configmaps Secrets**, the workflow I want to remember is:
...
```text
secrets, configuration, configmap, verify, change, kubectl, kubernetes, object, result, state, app-config, bash
```
...
> "The reason we use Configmaps Secrets here is because it gives us a repeatable way to handle configmaps secrets. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
