...
Deployments can update application versions gradually.
...
the cluster can replace old Pods gradually.

## Production concern

A rolling update can still fail if:

- new image is broken
- readiness probe is incorrect
- application is incompatible
- database migration is unsafe
- resource capacity is insufficient

Always think about rollback before deployment.
...
### Practical lab: deploy a new image and roll back
...
I verify the actual Pods after rollback rather than assuming the command alone means the application is healthy.
...
For **Rolling Updates Rollback**, the workflow I want to remember is:
...
```text
deployment, kubectl, rollout, application, bash, change, image, verify, result, rollback, rolling, state
```
...
> "The reason we use Rolling Updates Rollback here is because it gives us a repeatable way to handle rolling updates rollback. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
