...
Runs controllers that continuously work toward desired state.

## Worker node

### kubelet

Node agent responsible for ensuring containers described by Pods are running appropriately.
...
Provides communication between workloads and services.
...
Kubernetes is powerful because it continuously reconciles state rather than executing a deployment only once.
...
I use a disposable namespace/local cluster and start with `kubectl get nodes`, `kubectl get pods -A` and recent events. I inspect the controller, object, Pod, logs and dependencies in that order and deliberately break one component to practice troubleshooting.
...
For **Architecture**, the workflow I want to remember is:
...
```text
state, kubernetes, change, cluster, desired, object, result, state., verify, controller, controllers, events
```
...
> "The reason we use Architecture here is because it gives us a repeatable way to handle architecture. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
