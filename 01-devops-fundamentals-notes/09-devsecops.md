...
Security should not be something checked only at the end.

## Security can be added throughout the pipeline
...
Applications depend on third-party packages that may contain vulnerabilities.

### Container images

Images should be scanned and kept minimal.

### Infrastructure

Terraform changes should be reviewed.
...
Security is not only the security team's problem.

The engineer writing the pipeline, Terraform, Dockerfile or Kubernetes manifest can introduce or prevent vulnerabilities.
...
### Practical: add security checks to a pipeline
...
A security failure should provide actionable evidence, not just a red pipeline.

For example, if an image scanner reports a vulnerable package, I identify:
...
Then I rebuild and verify the finding is resolved.
...
For **Devsecops**, the workflow I want to remember is:
...
```text
scanning, security, change, image, result, text, verify, explain, only, terraform, before, changes
```
...
> "The reason we use Devsecops here is because it gives us a repeatable way to handle devsecops. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
