...
## Problems before DevOps practices

A traditional software delivery process can look like:
...
Every handoff can introduce delay and misunderstanding.

## Common problems

### 1. Manual deployments

Someone logs into a server and executes commands manually.
...
If testing happens days after development, fixing the defect becomes more expensive.

### 4. Configuration drift

Two servers that were supposed to be identical slowly become different because administrators make manual changes.

### 5. Difficult rollback

If a deployment fails, there may be no tested process for returning to the previous version.
...
```text
Manual → undocumented → inconsistent
```

we try to create:

```text
Automated → version controlled → repeatable → observable
```
...
Docker packages an application consistently.
...
Good DevOps practices can improve:
...
Because repeated manual work is both slow and error-prone. Automation turns a manual procedure into a repeatable process that can be version controlled, tested and executed consistently.

## Practical example

Suppose installing nginx requires:
...
Doing this manually on 50 servers is inefficient.

Ansible can describe the desired state and apply the same logic to all 50 systems.

That is the DevOps way of thinking.
...
For **Why Devops**, the workflow I want to remember is:
...
```text
devops, text, manual, result, verify, before, change, nginx, application, explain, process, time
```
...
> "The reason we use Why Devops here is because it gives us a repeatable way to handle why devops. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
