...
Changing random things until the problem disappears.
...
Prefer the smallest reversible change that can confirm your hypothesis.
...
### Practical: use a hypothesis log

For a failed deployment I write:
...
Evidence:
Previous container log shows missing environment variable.

Hypothesis:
Secret/configuration was not injected.
...
Verification:
Pod becomes Ready and smoke test succeeds.
```

This is much more useful than writing "fixed deployment" in a ticket.

I want my troubleshooting notes to be reproducible by another engineer.
...
For **Devops Troubleshooting Mindset**, the workflow I want to remember is:
...
```text
change, hypothesis, test, result, text, troubleshooting, verify, application, evidence, explain, symptom, before
```
...
> "The reason we use Devops Troubleshooting Mindset here is because it gives us a repeatable way to handle devops troubleshooting mindset. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
