...
2. Read logs.
3. Determine whether failure is code/build/test/image/deployment related.
4. Check recent changes.
5. Reproduce if safe.
6. Roll back if production is impacted.
7. Fix root cause.
8. Add prevention.
...
Then investigate CPU, memory, swap, disk, I/O, network and application logs.

## Scenario 4 — How would you automate 100 servers?

A strong answer:

> Use inventory-driven configuration management such as Ansible, keep the automation in Git, test it against a non-production group first, use idempotent tasks, control secrets, and validate the result.

## Scenario 5 — What makes a good DevOps engineer?

Not the number of tools memorized.
...
For **Devops Interview Scenarios**, the workflow I want to remember is:
...
```text
scenario, test, answer, change, check, explain, result, verify, devops, hypothesis, topic, understand
```
...
> "The reason we use Devops Interview Scenarios here is because it gives us a repeatable way to handle devops interview scenarios. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
