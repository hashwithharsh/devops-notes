# EBS, AMI and Snapshots

## EBS

Elastic Block Store provides block storage for EC2.

The operating system commonly uses EBS-backed volumes for persistent data.

## Snapshot

A snapshot captures a point-in-time backup of an EBS volume.
...
### Practical lab: inspect an EC2 volume

Find attached volumes:

```bash
aws ec2 describe-volumes   --filters Name=attachment.instance-id,Values=<instance-id>   --output table
```
...
```text
EBS volume → live block storage
snapshot → point-in-time backup/copy mechanism
AMI → image used to launch instances
```

Before deleting or replacing storage I verify whether the data is backed up and whether the resource has a dependency.
...
For **Ebs Ami Snapshots**, the workflow I want to remember is:
...
```text
storage, text, verify, before, change, does, only, result, snapshot, volume, backup, bash
```
...
> "The reason we use Ebs Ami Snapshots here is because it gives us a repeatable way to handle ebs ami snapshots. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
