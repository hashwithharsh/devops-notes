...
Security groups are associated with network interfaces/resources.

They are stateful.

If an allowed connection is established, return traffic is automatically allowed by the stateful behavior.
...
Do not check only the security group.
...
### Practical: troubleshoot a blocked connection

I define the exact flow first:
...
1. Security group on the destination.
2. Network ACLs on both subnet boundaries.
3. Route tables.
4. Host firewall.
5. Application listener.

I remember the practical distinction:

```text
Security Group → stateful, attached to ENI/resource
NACL → stateless, subnet boundary
```

This keeps me from treating both as interchangeable firewalls.
...
For **Security Groups Vs Nacl**, the workflow I want to remember is:
...
```text
security, check, group, nacl, network, only, text, change, does, groups, result, service
```
...
> "The reason we use Security Groups Vs Nacl here is because it gives us a repeatable way to handle security groups vs nacl. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
