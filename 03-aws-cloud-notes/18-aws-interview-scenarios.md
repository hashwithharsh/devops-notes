...
> I would first verify the instance subnet and route table, then check whether the subnet is intended to be public or private. For a public instance I would verify the Internet Gateway route and public addressing. For a private instance I would verify the NAT path. Then I would check security groups, NACLs and DNS.

## Scenario: User gets AccessDenied

Answer:

> I would identify the caller using STS, inspect applicable identity and resource policies, check for explicit denies and conditions, and verify the account/resource ARN.
...
## Scenario: Application needs S3

Do not immediately create an access key.
...
## Scenario: Need high availability

Use multiple Availability Zones and remove single points of failure where practical.

## Scenario: Need scalable web application
...
with appropriate subnets, security controls and monitoring.
...
For **Aws Interview Scenarios**, the workflow I want to remember is:
...
```text
verify, would, scenario, check, answer, application, security, text, change, does, hypothesis, only
```
...
> "The reason we use Aws Interview Scenarios here is because it gives us a repeatable way to handle aws interview scenarios. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
