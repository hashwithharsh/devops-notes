...
Primarily an availability/failover concept.

### Read replica

Primarily used for read scaling and certain reporting/read workloads.

Do not answer:

> Multi-AZ is for read scaling.
...
### Practical: reason about application-to-database connectivity
...
I do not make the database public simply because an application cannot connect.
...
DB is available?
correct port?
application credentials correct?
```

For Route 53 I remember that DNS maps names to destinations; it does not itself create network connectivity.
...
For **Rds Route53**, the workflow I want to remember is:
...
```text
application, does, read, route, security, text, verify, change, only, result, answer, before
```
...
> "The reason we use Rds Route53 here is because it gives us a repeatable way to handle rds route53. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
