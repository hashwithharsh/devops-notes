...
If you do, you lose the ability to identify which change fixed or caused the problem.
...
### Practical AWS troubleshooting sequence

When something fails I collect:
...
Then I trace the relevant dependency graph.

For networking:

```text
DNS → route → SG/NACL → host firewall → listener
```

For IAM:

```text
caller → action → resource → policy evaluation
```

For EC2:

```text
instance → ENI → subnet → route → storage → OS → service
```

This structure keeps me from changing several unrelated resources at once.
...
For **Aws Troubleshooting**, the workflow I want to remember is:
...
```text
text, change, check, troubleshooting, before, does, only, resource, result, route, security, service
```
...
> "The reason we use Aws Troubleshooting here is because it gives us a repeatable way to handle aws troubleshooting. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
