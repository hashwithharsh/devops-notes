...
Provides the operating system/application image used to launch the instance.

## User data

User data can execute initialization commands during instance launch.
...
For repeatable infrastructure, prefer IaC/configuration management rather than increasingly complex user-data scripts.

## Instance states

Running, stopped and terminated have different operational implications.
...
Termination is destructive to the instance resource.
...
### Practical lab: inspect an EC2 instance with CLI
...
For one instance I check:

```bash
aws ec2 describe-instances   --instance-ids <instance-id>
```

Then I trace its architecture:
...
If SSH fails, I do not only check the key. I check network path, security group, NACL/route, instance state and the SSH service on the guest.
...
For **Ec2 Deep Dive**, the workflow I want to remember is:
...
```text
instance, text, check, only, security, service, change, does, network, result, state, verify
```
...
> "The reason we use Ec2 Deep Dive here is because it gives us a repeatable way to handle ec2 deep dive. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
