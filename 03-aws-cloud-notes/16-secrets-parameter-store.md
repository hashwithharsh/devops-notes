...
If an application needs one secret, its IAM role should not automatically receive access to every secret.

## Pipeline warning

Avoid printing secret values in CI logs.

Even if the CI system masks common secret formats, accidental transformations or shell commands can still leak sensitive data.
...
### Practical: retrieve configuration without hard-coding it

I distinguish:

```text
normal configuration → Parameter Store
sensitive secrets → Secrets Manager or another approved secret system
```

The application should receive access through IAM permissions/role-based identity rather than hard-coded AWS keys.

When troubleshooting, I verify the application identity has permission to read the specific parameter/secret and that the name/Region are correct.
...
For **Secrets Parameter Store**, the workflow I want to remember is:
...
```text
application, parameter, secret, secrets, text, verify, access, change, does, manager, only, result
```
...
> "The reason we use Secrets Parameter Store here is because it gives us a repeatable way to handle secrets parameter store. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
