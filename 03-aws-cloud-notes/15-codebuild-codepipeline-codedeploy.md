...
CodeBuild performs build/test steps.

Common concept:

```text
Source → CodeBuild → Test → Artifact
```

A `buildspec.yml` can describe build phases.
...
CodeDeploy automates deployment to supported targets.
...
They can be combined into one delivery architecture.

## Modern AWS + GitHub architecture
...
### Practical: understand AWS CI/CD roles

I keep the responsibilities separate:

```text
CodeBuild → build/test
CodePipeline → orchestrate stages
CodeDeploy → deployment mechanism
```

If I use GitHub as source, I still trace the complete path:
...
For a failed pipeline I identify the first service/stage that failed before troubleshooting the downstream deployment.
...
For **Codebuild Codepipeline Codedeploy**, the workflow I want to remember is:
...
```text
codebuild, text, build, codedeploy, codepipeline, test, deployment, github, before, change, does, only
```
...
> "The reason we use Codebuild Codepipeline Codedeploy here is because it gives us a repeatable way to handle codebuild codepipeline codedeploy. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
