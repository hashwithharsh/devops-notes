...
> What AWS API activity happened?

Both are useful during incidents.
...
### Practical: investigate an AWS event

CloudWatch answers operational questions such as:

```text
Is CPU high?
Is a load balancer returning errors?
Did an application emit an error?
```

CloudTrail answers a different class of question:
...
When investigating an unexpected infrastructure change, I use CloudTrail to establish the control-plane event and CloudWatch/application logs for runtime behavior.
...
For **Cloudwatch Cloudtrail**, the workflow I want to remember is:
...
```text
cloudwatch, change, cloudtrail, text, does, only, result, useful, verify, answer, application, before
```
...
> "The reason we use Cloudwatch Cloudtrail here is because it gives us a repeatable way to handle cloudwatch cloudtrail. The practical part is that I can inspect the current state, make the intended change, verify it, and troubleshoot it without relying on manual guesswork."
